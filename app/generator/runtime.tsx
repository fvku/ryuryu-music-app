...
import { generatorRemoteImageUrl, releaseMasterCover } from "@/lib/generator/cover-source";
 ...
 * 外部カバーは認証付きの同一Origin経路へ向け、CORSに依存せずcanvasを汚染しない。
 */
function coverMaps(albums: ReleaseMasterAlbum[]) {
  ...
  for (const album of albums) {
    const cover = generatorRemoteImageUrl(releaseMasterCover(album) || "");
    if (!cover) continue;
...
export function jacketSource(runtime: GeneratorRuntime, documentId: string, slot: CanvasPreviewPage["slots"][number]): string | null {
  return (slot.jacketAssetId && assetUrl(documentId, slot.jacketAssetId))
    || generatorRemoteImageUrl(slot.sourceCoverUrl || "")
