...
/** アルバムを聴けるサイトへのリンクボタン。Spotify URLならSpotify、それ以外（Bandcampなど）は汎用表示にする。 */
export default function ListenLinkButton({ url }: { url?: string }) {
  if (!url) return null;
  const bandcamp = isBandcampUrl(url);
  ...
  return (
    <a
      ...
      style={{ backgroundColor: bandcamp ? "#1da0c3" : "#1db954", color: "white" }}
    >
      {bandcamp ? (
      ...
      {bandcamp ? "Bandcampで聴く" : "Spotifyで聴く"}
    </a>
  );
}
