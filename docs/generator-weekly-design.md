...
- Other Releasesの並びは**アルバム（洋楽→邦楽）→ EP（洋楽→邦楽）**で、各区分の中は**アーティスト名のa-z順**（Koheiの指示、2026-09-11に確定）。実運用のEPは邦楽なので、見た目は「洋楽アルバム→邦楽アルバム→邦楽EP」になる。EPは `[EP]` プレフィックスを**残す**（Monthly／Japanは作品名から `[EP]` を外すので、ここが逆になる）。
...
2. featureは日付・アーティスト名の既存規則（`sortAlbums`）で並べる。othersは`sortWeeklyOthers`で「アルバム（洋楽→邦楽）→ EP（洋楽→邦楽）、各区分の中はアーティスト名のa-z順」に並べる（日付は見ない）。どちらもEPは末尾へ送り、**作品名から`[EP]`は外さない**。
   洋邦は`洋邦`列の値で判定し、空欄・想定外の値はその区分の末尾へ置く。アーティスト名は小文字にしてコードポイントで比較するので、記号・数字が先、和文はA-Zの後に来る。
...
| パネル（タイトル・アーティスト・メタをまとめて1枚） | **(200,856) 800×294**。**罫はこの外側6px**（Monthlyと同じ`strokeRect`の描き方）。2026-09-11訂正：塗りの始まりはy856で、それより上（y850–856）は白罫。以前は(200,850)800×300としていたため塗りが白罫へ重なり、この1本だけ白が40%に沈んでいた |
...
**🔴2026-09-17：字間≠0のときペアカーニングが消えていた不具合を修正。** `tools/generator-lab/core/textEngine.mjs`
の`prepare()`／`prepareMixed()`は、字間が0以外のとき1文字ずつ`ctx.measureText(ch)`で測って描いていたため、
隣接文字どうしのペアカーニング（例: 大文字直後の詰め）がブラウザ側で失われ、字間0でも字間≠0の版と
同じ幅になっていた（実共有文書「2026 WEEK 37」の実測で「Tyber」が実物98.5pxに対し102.0pxと判明。
docs/generator-weekly-w37-diff-plan.md §2 A4）。先頭からの累積幅の差分で各文字の送り幅を求める方式へ直し、
99.0pxまで縮まった（残差0.5px）。**Monthly／Japanの字間≠0の作品名にも同じ影響がある**ため、あわせて直した。

**🔴2026-09-17：Oswaldに無い記号（∞等）が作品名に出ると、その1文字だけ意図せずNoto Sans JPへ
差し替わっていた。** `TextEngine.toClusters()`の文字分類（`LATIN`正規表現）にU+221E（∞）が入っておらず、
`isMixed()`が「和文が混じる」と誤判定して`drawMixedSingle()`（和欧混植）の経路へ回っていたため。
実際はOswald側にも∞のグリフはあるが、Google Fontsの標準サブセット取得には含まれず未読み込みだった
（2026#37 Sylvan Esso「Ow ∞」で実測、Noto代用は実物比+18%幅・別字形。§2 A3）。
`LATIN`へ∞を追加し、`fonts.mjs`に`text=`指定でOswaldの記号グリフだけを追加取得する`loadSymbolFaces()`
を足した（該当記号のみが対象。読み込みに失敗しても他の描画は止めない）。実測で幅103.5pxとほぼ完全一致。
新しい記号が見つかったら`fonts.mjs`の`SYMBOLS`定数へ追記する。
...
**🔴2026-09-17：帯の顔検出（§9-1・§30の顔検出クロップ）の採否を保守的な規則へ直した。**
実共有文書「2026 WEEK 37」を実物投稿と突き合わせると、抽象画（人物なし）を確信度0.42〜0.50で
顔と誤検出して152pxもずらした例（Bonobo「Distance in Static」）があった。旧`MIN_SCORE=0.3`は
これも採用してしまう。W36・W37の実物10枚で検算し直すと、**確信度0.7以上だけを採用**する規則で
9/10枚が実物の切り抜き位置と一致した。あわせて、確信度0.7以上の顔が複数・離れた位置に見つかった
場合（集合写真）は中央へ戻すようにした——1人だけを追って残りのメンバーの顔を帯の外へ追い出すのを
避けるため。Koheiの承認（2026-09-17、 askで確認済み）で実装。詳細はdocs/generator-weekly-w37-diff-plan.md
§2 A5・§3.1-5、実装は`tools/generator-lab/core/face-crop.mjs`の`MIN_SCORE`・`CLUSTER_WIDTH`・
`GROUP_SPREAD`。**手動の微調整はまだ無い**（§9-1に既述のとおり、Codexへの相談として残したまま）。
...
| 見出し「Other Releases」 | Oswald Regular **72px**（2026-09-14訂正、旧71px）、中央揃え、ベースライン**169.5** | 🔵実測 |
| 本文の書式 | `"作品名 / アーティスト名"`の1行（区切りは半角スラッシュ）。`[EP]`は保持（Monthlyと逆） | 🔵実測 |
| 本文フォント | Oswald Regular **26px・字間+5%**（2026-09-17再訂正。旧29px→2026-09-14に31px→今回26pxへ。下記参照） | 🔵実測 |
| 行送りの自動調整域 | ボックス(240,222) 720×**870**を行数で単純に割る（`floor(870/n)`、2026-09-17訂正。下記参照） | 🔵実測 |
...
**🔴2026-09-17再訂正：本文の書体と行送りの式を再実測で直した。** 実共有文書「2026 WEEK 37」を実物投稿と
突き合わせると、2026-09-14の「weight 300・size 31」は幅と塗りの量だけで照合しており、**字高**
（キャップハイト）を見ていなかった。実物の字高は21px、旧設定は24〜25pxと15〜19%大きかった——
「Light 31px・字間0」と「Regular 26px・字間+5%」は幅も塗りの量もほぼ同じになるため、幅と塗りだけの
照合では区別できなかった。2026#36・2026#37計9行を幅・塗り量・字高の3点で総当たりし、**Oswald
Regular 26px・字間+5%**（字高21pxで一致、幅差1px未満、塗り差+1.6%）に確定した。和文（Noto Sans JP）
も同じ26px・字間+5%で幅が全行±1.5px以内に収まる。

行送りも同時に直した。実物W37（28行）はベースラインが**31.0px刻み**（`floor(870/28)`）で完全に
等間隔だった。W36（30行）は29.0px刻み（`floor(870/30)`）。旧式（先頭・末尾を固定して行数-1で
均等割り）は行数が少ない週ほど最終行が下へずれる（W37で3.5px）。`Layout.WEEKLY.OTHERS.layoutFor(n)`
を「天地の可用域870pxを行数で単純に割り、上端から並べる」式へ変更し、`MIN_LEAD`も本文サイズと同じ
26pxへ下げた（最大収容行数は30→33行）。詳細はdocs/generator-weekly-w37-diff-plan.md §2 A1・A2。
...
- 2026-09-11 Koheiの指示でOther Releasesの並びを`sortWeeklyOthers`（アルバム 洋楽→邦楽 → EP 洋楽→邦楽、各区分内はアーティスト名a-z）に変更。
  設計文書は当初から「洋楽→邦楽」と書いていたが、実装は`sortAlbums`（EP→日付→アーティスト）を共用しており、実際は発売日順に並んでいた。
- 2026-09-11 ジャケットとパネルのあいだの白罫が半透明に見える不具合を修正。パネルのセルを(200,850)800×300から
  **(200,856)800×294**へ。塗り（黒60%）がジャケット側の下罫(850–856)へ重なっていたのが原因で、
  実物投稿4枚（W-1・W-2・W-3・W-5）の画素で「白罫は850–856の6px1本、塗りはy856から」を確認して確定した。

- **2026-09-14 Koheiの目視指摘を受け、`tools/generator-lab/reference/2026#37`（実物投稿7枚）で再実測し、
  複数の差分を修正した。** 経緯：「表紙のジャケット切り取りを顔中心にできるか」「Other Releasesが太い」の
  2点の指摘から、実物投稿とPNG書き出し経路（1200基準×2倍×`EXPORT_SUPER_SAMPLE`）で描いた候補を
  インクの幅・塗り量・位置で総当たり照合する検証ツール
  （`tools/generator-lab/weekly-weight-check.html`・`face-crop-check.html`、いずれも未コミット→今回コミット）
  を作り、2026#36・2026#37の両方で測り直した。見つかった差分：

  1. **Other Releases本文が太い（ご指摘どおり）。** 2026-09-09に入れた「`renderWeight:350`で描く太さだけ
     下げる」補正は、`Render.drawWeeklyOthers()`が`TextEngine.drawSingle()`へ`opt.renderWeight`を
     渡していなかったため**一度も効いていなかった**（実際はweight 400のまま描かれ続けていた）。
     さらに、当時「Regular 29px」と測っていた本文の書体指定そのものが誤りで、9行（2026#36・2026#37の
     計9箇所）の総当たり実測では**weight 300・size 31**が幅差±2%・塗り差±3%以内でどの行も一致した
     （29px Regularと31px Lightは文字幅がほぼ同じで、幅だけの実測では見分けられなかったとみられる）。
     `renderWeight`の小細工は不要になったため廃止した。見出し「Other Releases」も71px→**72px**へ訂正
     （幅差1px未満）。`Layout.WEEKLY.OTHERS.TYPE.body`・`TYPE.heading`を修正。
  2. **本文の収まる最小行送り（`fits()`の下限）を、サイズに連動しない独立の定数へ切り離した。**
     上記1でTYPE.body.sizeを29→31にすると、`fits()`が「lead ≥ TYPE.body.size」のままでは
     実際に30行が収まっているW36が「収まらない」と誤判定してしまう（841/29=29.0 < 31）。
     下限そのものは動いていない（30行が収まる＝29pxが境界という実測は変わらない）ので、
     `Layout.WEEKLY.OTHERS.MIN_LEAD = 29`という独立の定数に切り出した。
  3. **メタ帯のベースラインが1096ではなく1092.5だった。** 2026-09-08の実測は行全域を1本の帯として
     下端を拾っており、`"12SONGS,"`のようなコンマのディセンダ（下がり）に引きずられて1096まで沈んでいた。
     列ごとのインク下端の最頻値（コンマ等の外れ値を1列単位で無視できる）で測り直すと、
     2026#36・2026#37の計6面すべてでy1092が最頻値・y1093が僅差の2位という同じ分布になった。
     中間の1092.5を正とし、`META_CELL.y`も1067→1063.5へ連動して直した
     （`y + h/2 + 13 = META_BASELINE`の関係式は維持）。
  4. **表紙の帯（ご指摘どおり）：人物の顔を自動で中心に寄せられるようにした。** 帯は正方形ジャケットを
     高さでcover-fitし、横は中央240pxだけを見せる仕組みなので、人物が中央からずれていると顔が帯の外へ
     落ちる。`tools/generator-lab/core/face-crop.mjs`を新規追加し、MediaPipe Tasks Vision
     （BlazeFace short-range）をCDNから実行時に動的読み込みして顔の水平中心を検出、
     `Render.drawCover()`に追加した`focusX`引数（既定0.5＝中央、従来と数学的に同一）へ渡す。
     2026#36・2026#37の実物投稿10枚（ジャケット）で検証したところ、**9枚が実際の手作業の切り抜き位置と
     一致した**（残る1枚＝Dizzee Rascal「We Want Bass」は暗い夜景の小さな顔を拾えず中央のまま）。
     検出できない・確信度が低い・4秒でタイムアウトのいずれも中央フォールバックにし、表紙の生成自体は
     止めない。`app/generator/runtime.tsx`の`preparePage()`が表紙ページのジャケットだけ検出を走らせ、
     結果はジャケットのURLごとにキャッシュする（同じジャケットで検出をやり直さない）。
     **自動検出が外れた作品を人が直す手段（手動の微調整）はまだ無い。** 文書への保存が要るため、
     `docs/codex-generator-handoff.md`へCodexへの相談として残した。
  5. **Other Releasesの並び順で、和文アーティストの照合が文字コード順になっていた。** `sortWeeklyOthers`の
     アーティスト比較を`Intl.Collator('ja')`へ変更。2026#37の実物投稿（邦楽区分：OZworld→ハク。→
     ポルノグラフィティ→斉藤和義→福山雅治→幽体コミュニケーションズ→緑黄色社会）を単純な
     コードポイント比較では再現できなかったが、`Intl.Collator('ja')`に掛け直すと完全に一致した
     （漢字は読みを持たないため厳密な五十音順ではないが、CLDRの日本語照合規則に沿った並びが実物と
     一致している）。`sortAlbums`（Monthly／Japan／Weekly featureの日付優先の並び）は変更していない
     （アーティスト名は同日タイブレークにしか使わないため）。

  検証は`node --test tools/generator-lab/test/*.mjs`（61件）・`npx vitest run`（407件、新規2件を含む）・
  `npx tsc --noEmit`・`npm run lint`・`npm run build`をすべて通した。ローカル本番プレビュー
  （`generator-preview-3458`、Googleログイン迂回）で実共有文書「2026 WEEK 37」の表紙・Other Releasesを
  実描画し、顔検出が実際にMediaPipeのWASMグラフを起動して動作すること、表紙2バンド
  （2位Ezra Collective・1位Tyber/FKJ）で顔が実物と近い位置に寄ることを画素で確認した。
  共有DBへの書き込みは行っていない（閲覧のみ）。

  罫・パネル塗り・表紙オーバーレイ色（#0040C7@60%）・帯の並び順（`RANK_TO_BAND`）・波の合成（Luminosity
  50%）は2026#37でも差分なし。

- **2026-09-17 実共有文書「2026 WEEK 37」を実物投稿ともう一度突き合わせ、5件の差分を修正した。**
  経緯・実測根拠は`docs/generator-weekly-w37-diff-plan.md`、Koheiとの確認は同文書§4（ask questionで
  4点とも確認済み：行送り式・作品名の初期字間0維持・顔検出の保守化・HUGは「Release Masterが正、
  掲載になっていなければ載せない」）。
  1. **Other Releasesの本文をweight 300・size 31→weight 400・size 26・字間+5%へ再訂正**（§6.3）。
     2026-09-14の実測は幅と塗り量だけで照合しており字高を見落としていた。
  2. **Other Releasesの行送りを「先頭・末尾を固定して行数-1で均等割り」→「天地870pxを行数で単純に
     割る（`floor(870/n)`）」へ変更**（§6.3）。行数が少ない週ほど最終行が下へずれる不具合を修正。
     `MIN_LEAD`も26pxへ下げ、最大収容行数が30→33行になった。
  3. **字間≠0のときペアカーニングが消えていた不具合を修正**（`TextEngine.prepare()`／
     `prepareMixed()`）。Monthly／Japanの字間≠0の作品名にも影響する一般修正（§6.1）。
  4. **OswaldにあるのにMonthly/Weeklyの読み込みには含まれない記号（∞等）が、Noto Sans JPへ
     意図せず差し替わっていた不具合を修正**（`TextEngine`の文字分類＋`fonts.mjs`の追加読み込み。§6.1）。
  5. **表紙の帯（顔検出クロップ）の採否を、確信度0.7未満は不採用・複数の顔が離れていたら中央、
     という保守的な規則へ変更**（§6.2）。抽象画を顔と誤検出していたBonoboの152pxのずれが解消した。

  検証は`node --test tools/generator-lab/test/*.mjs`（61件）・`npx vitest run`（407件）・
  `npx tsc --noEmit --incremental false`・`npm run lint`・`npm run build`をすべて通した。
  ローカル本番プレビュー（`generator-preview-3458`）で実共有文書「2026 WEEK 37」の全7枚を書き出し、
  実物投稿と画素で突き合わせて確認した（共有DBへの書き込みなし、閲覧のみ）。W-4（Ow ∞）は
  全画面平均差0.98→0.73階調、表紙は10.00→6.39階調まで縮まった。W37文書のデータ差分
  （B1〜B8：ジャンル・国の欠落、ジャケット解像度、地の色、字間、HUGの扱い、並び順など）は
  共有DBへの書き込みが要るためKoheiの操作に委ね、今回は直していない。
