...
更新日：2026-09-09。**既存プロトタイプのSupabaseへ初期SQLと追加マイグレーションを適用し、非公開バケット、ローカル開発接続、Vercel Production接続を設定済み。ローカル・本番画面ではRelease Master取り込み、共有文書、全項目編集、対象別ロック・保存・履歴復元、画像アップロード、PNG書き出しを利用できる。** Previewには本番DBの接続情報を登録していない。最新の適用状況・保全データ・残件は[DB再利用の記録](./generator-project-reuse.md)を参照。
...
本番の実共有W36文書を3人が別端末・別Googleアカウントで開き、開始時の文書versionを記録する。
モバイルUIは再検討対象のため、この確認はPCまたはiPadでもよい。

1. 3人がそれぞれ異なる作品を開き、他の2人の対象に編集者表示が出ることを確認する。
2. 各自が作品名の末尾へ一時的な識別文字を入れ、ほぼ同時に保存する。3件すべてが残り、文書versionが3進むことを確認する。
3. 1人が作品を編集中に、別の人が同じ作品を開く。後者が編集できず、編集者名と競合表示が出ることを確認する。
4. 先の人が別作品へ移動した後、後の人が元の作品を取得・保存できることを確認する。
5. 1人が編集中のタブを閉じ、3分を超えてから別の人が同じ作品を取得できることを確認する。
6. 同じGoogleアカウントの2端末で同じ作品を開き、後の端末で「この端末へ引き継ぐ」を実行する。旧端末の保存が拒否され、新端末だけが保存できることを確認する。
7. 1作品を履歴から開始時点へ復元し、ほかの2作品の変更が巻き戻らないことを確認する。
8. 残る2作品も開始時点の内容へ対象別に復元し、最終的に未保存0件・ロック0件であることを確認する。

実施記録には、参加者、端末・ブラウザ、開始／終了version、各手順の合否、競合時の表示文言を残す。

### 2026-09-09の接続分離試験

- PGlite上で、3人の別対象同時保存、同一対象の競合、期限切れ、同一人物の端末引き継ぎ、対象別履歴復元を含む19件が成功した。
- 同じMac上に3つのlocalhost Originを立て、Kohei・Meri・Hanawaを別actor／別clientIdとして実共有W36文書へ接続した。
- version 8から、`We Want Bass`、`I Know Too Much`、`ACT III`の3作品をほぼ同時に保存した。Hanawa→Kohei→Meriの順でversion 9〜11となり、最新版に3件すべてが共存した。
- 他actorが保持する作品では編集欄が無効化され、サムネイルの「他が編集中」と編集者メールを確認した。
- 同じKohei actorの2クライアントで編集権を引き継ぎ、旧クライアントの保存が「他の変更が先に保存されました」と拒否され、共有DBが変更されないことを確認した。
- 3作品をversion 8の内容から対象別に復元し、version 12〜14として記録した。ほかの作品を巻き戻さず、元の作品名へ戻った。
- 画面から企画一覧へ遷移してロックを解放し、最終状態はversion 14、すべて保存済み、ロック0件。試験用のローカル復旧コピーも破棄した。

これにより、実Supabase・実HTTP API・実UIを通る3主体の接続分離は確認済み。
残るのは物理的に異なる3端末での受入と、タブを突然閉じた場合に3分後の期限切れが実時間でも成立することの確認。
モバイルUIは再検討対象のため、別端末試験はPCまたはiPadで実施してよい。
