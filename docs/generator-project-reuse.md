...
- 追加SQLは適用トランザクション内で既存データのハッシュ不変を確認した。実DBの合成トランザクションで構成保存と画像準備・ready・読取を確認してロールバックした。
...
2つのマイグレーションは**既に適用済み**なので、編集・再適用しない。以後の変更は新しい追加マイグレーションとこの適用記録を照合して進める。
