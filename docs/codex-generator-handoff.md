...
統合版には全ページの一括書き出しがない。旧スタンドアロン版にあった「全部書き出す」と
`editor/js/zip.js` が引き継がれていなかった。Monthly 1か月ぶん9〜14枚を1枚ずつ押す状態になっている。

2026-09-07に、**UI・逐次描画・進捗・中止までをClaude Code側で用意した**。残りは1点だけ。

1. **`app/generator/bulk-export.ts` の `getArchivePacker()` を実装してほしい。**
   現在は常に `null` を返し、ボタンは「全ページを書き出す（準備中）」で押せない状態にしてある。
   `(entries: {name, blob}[]) => Promise<Blob>` を返せば、UI側は変更なしで動く。
   ...
   実装したら `lib/generator/__tests__/bulk-export.test.ts` の
   「まとめ役はまだ実装されていない」テストを削除してほしい。
2. **はみ出しがあるページの扱いは仕様判断**。現在は「全ページを先に検査し、1枚でも警告があれば
   何も作らずに止め、画像番号と理由をまとめて出す」で実装した。部分的に書き出して
   「9枚のはずが7枚」になる方が危険だと判断したもの。この解釈でよいか確認し、決めた内容を仕様書§7へ追記してほしい。
3. **メモリと所要時間の受入**。2400px×9〜14枚の逐次生成を実機で確認してほしい。
   1枚ごとに `releaseCanvas` している（失敗時も `finally` で解放、テストあり）が、実機での上限は未確認。
