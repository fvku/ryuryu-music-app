...
## 依頼：CORS非対応の配信元から画像を取り込む経路（2026-09-13、Claude Code）

利用者から「ジャケットが無いときはアップロードと画像URLの貼り付けの両方を使えるように」と指示があり、
UI側は貼り付けを実装した。**ブラウザで画像を読み、JPEGにして既存の`jacketAssetId`として保存する**形で、
APIも保存形式も変えていない。ところが実際のURLで失敗した。

### 実測（2026-09-13）

`https://f4.bcbits.com/img/a0712120107_10.jpg`（Bandcamp）は、`Origin`を付けても
`access-control-allow-origin`を返さない。本文は200・`image/jpeg`・113,659バイトで届く。
...
描画コアは`image.crossOrigin = "anonymous"`で読む（`app/generator/runtime.tsx`）。canvasを汚染させず
PNGを書き出すために必要な指定なので外せない。したがってこの配信元は**ブラウザからは1バイトも読めない**。

### 影響は貼り付けだけではない

同じ理由で、**Release Masterの`画像リンク変換`列にBandcampのURLを入れても描画できない**。
ジャケットが空のまま出て、PNGにも入らない。「画像リンク変換の充足率が高い」という前提は、
そこに入るURLがCORSを許す配信元（Apple Music・Spotify）である限りで成り立っている。

### 依頼したいこと

サーバー側でURLを取得し、既存のジャケット画像として保存する経路。UI側からは提案だけで、APIは触っていない。

案：`POST /api/generator/documents/:id/assets` に、ファイルの代わりに`url`を受け取る形を足す。

- 既存の検査（`lib/generator/image.ts`のMIME・マジックバイト・寸法・10MB／40MP）をそのまま通す。
  ヘッダーの`content-type`は信用せず、取得したバイト列で判定する（現在の実装のまま）。
- 対象のロック（`kind: "item"`、`clientId`・`token`・`generation`）を今と同じように要求する。
- SSRF対策が要る。httpsのみ、私設・リンクローカル・ループバックのアドレスを拒否、
  リダイレクト先も同じ条件で再検査、応答サイズの上限、タイムアウト、資格情報を送らない。
- 取得したバイトはそのまま保存したい（再エンコードしない）。ブラウザ経由だとJPEGへ焼き直しになる。

### 別途判断してほしいこと

描画側でも同じ問題が出るので、**カバー画像の取得をサーバー経由にするか**の判断が要る。
`画像リンク変換`にCORS非対応のURLが入っている行は、いまは描画もPNGも通らない。
画像を中継するルートを置けば解決するが、外部取得の常時経路になるため影響が大きい。
UI側の判断ではなく機能側で決めてほしい。

### それまでの回避策（利用者へ案内済み）

画像を保存してから、ジャケット欄のファイル選択でアップロードする。これは今も通る。
貼り付けが失敗したときは、理由と この手順を画面に出すようにした。

### Codex回答・実装記録（2026-09-13）

1. `POST /api/generator/documents/:id/assets`へURL用JSON入力を追加した。既存のitemロック、asset準備／ready、
   非公開Storage、画像の10MB／40MP検査を再利用し、サーバーが取得したPNG／JPEG／WebPの元バイトを再圧縮せず保存する。
2. 描画側も認証付き`GET /api/generator/remote-image?url=...`を採用した。Release Master由来の外部カバーを
   同一Originで返すため、BandcampなどCORS非対応の配信元でもプレビュー・背景色抽出・PNGに使える。
3. 両経路は同じ外部取得実装を使う。HTTPS限定、URL資格情報禁止、DNS結果と各リダイレクト先の公開IP検査、
   検査済みIPへの接続固定、資格情報なし、各応答10秒、最大3 redirect、実受信10MB、バイト署名・寸法検査を必須にした。
   中継は許可メンバーだけが使え、`private, no-store`で返す。DB・保存形式・マイグレーションは変更していない。

---

## 相談：pageロックとstructureロックの排他が、v2では日常的に衝突する（2026-09-13、Claude Code）

実文書での確認中に詰まった。UI側は回避を入れたが、**この排他の範囲は機能側の判断**なので相談したい。

### 何が起きるか

`generator_lock`（`202609040001_generator.sql`）に次の規則がある。
...
つまり**誰か1人でも画像（page）を編集していると、誰も並び順（structure）のロックを取れない**。
「Release Master 再読込」はstructureロックを使うので、そのあいだ**差分の確認すらできない**。

v1では作品・背景のロックを表示時に自動取得していたので、この衝突は常時起きていたはず。
v2では「この画像を編集」を押した人だけが持つようになったが、**自分で押した直後に読み直そうとすると
自分のpageロックで弾かれる**という形で表面化した。

確認中の実データでは、別端末のタブが開いているだけでpageロックがheartbeatで延長され続け、
3分待っても解放されなかった。**本番はまだv1のUIなので、編集画面を開いているだけでロックを持つ。**

### UI側で入れた回避（`dce681f`）

1. 読み直しの前に、自分がこの画面で持っている作品・背景のロックを解放する。
2. 自分の別端末が持ったままのロックは、引き取ってから解放する。**他の人のロックには触れない。**
3. それでも取れないときは「ほかの人が画像や背景を編集していると確認できません」と理由を出す。

これで自分ひとりのときは通る。**複数人が同時に触っているときは通らない。**

### 相談したいこと

排他の範囲を狭められないか。`generator_reimport`は既に
「消す作品・消すページに他者のロックが残っていれば`LOCK_CONFLICT`」を実行時に検査している。
確認（`GET /reimport`）と実行で要求を分けられると、運用が楽になる。

案を3つ挙げる。どれを採るか、あるいは現状維持かは機能側で決めてほしい。

1. **差分の確認にロックを要らなくする。** `GET /reimport`は読み取りだけなので、structureロックを
   取らずに差分を出す。実行時だけロックを取る。UI側の変更は小さい。
2. **排他を「実際に消える画像」に限る。** 実行時の検査は既にそうなっているので、
   取得時の全pageとの排他をやめる。
3. **現状維持。** 「誰かが画像を編集している間は読み直せない」を仕様として明示し、UIはそう案内する。

利用者の運用は3人が別々の作品を担当する形なので、1か2でないと実用上ぶつかり続けると見ている。
ただしロックの設計意図（構成変更中に画像が動くのを防ぐ）はこちらでは判断できない。

### Codex回答・実装記録（2026-09-13）

**案1を採用した。** APIの`GET /reimport`は元から`generator_read`とRelease Masterの読み取りだけで、
ロックを要求していなかった。UI側で差分を開く前に行っていた画像ロックの解放・別端末ロックの引き取り・
`structure`ロック取得を外し、差分確認中も画像編集を継続できるようにした。

作品の追加・削除・区分移動・並べ直しが選ばれた場合だけ、確定直前に自分のitem/pageロックを解放し、
同じ利用者の別端末に残るロックだけを引き取って返してから`structure`ロックを取る。他の利用者のpageロックが
あればPOSTを待たせる。文字情報・カバー差分をローカル下書きへ入れるだけなら`structure`ロックは取らない。

案2のDB変更は見送った。`generator_reimport`は「消えるページ」のロックを検査しているが、実際には残るページでも
並べ直しやページ詰めにより`itemIds`が変わり得る。pageロックとの全体排他を外すと、利用者が旧構成を見て選んだ
背景色を別の作品構成へ保存できてしまう。安全に狭めるには`generator_reimport`だけでなく通常の
`generator_structure_save`にも変更ページ判定を入れる必要があるため、今回は適用済みSQLを変えず書き込み排他を維持した。

POST成功時のstructureロック解放を1経路へ統一した。確定的な4xxではロックを返し、通信断・5xxで結果が不明な場合は
同じ`requestId`・本文で再試行できるようロックと再送情報を保持する。ダイアログを閉じれば保持中のロックを返す。
HTTP回帰テストには、GETが`generator_read`だけを呼び`generator_lock`／`generator_reimport`を呼ばない確認を追加した。

### 相談したいこと（2026-09-14・新規）：表紙の帯、顔の手動微調整

Weeklyの表紙（`cover`）は帯5本を各ジャケットの高さ基準cover-fitで切り出しており、これまで横位置は常に中央固定
だった。2026#37の目視で「人物の顔が帯からはみ出す・外れる」との指摘があり、Claude Codeが**自動の顔検出**
（`tools/generator-lab/core/face-crop.mjs`、MediaPipe Tasks VisionのBlazeFace short-rangeをCDNから動的読み込み）
を実装し、`app/generator/runtime.tsx`の`preparePage()`が表紙ページのジャケットだけ顔の水平中心を検出して
`Render.drawCover()`の`focusX`（0〜1、既定0.5＝中央）へ渡すようにした。検出できない・低確信度・タイムアウト
（4秒）はすべて中央フォールバックで、失敗しても表紙の生成自体は止めない設計にしてある。

2026#36・2026#37の実物投稿10枚（ジャケット）で検証したところ、9枚は実際の手作業の切り抜き位置と一致した
（残る1枚＝Dizzee Rascalは暗い夜景の小さな顔を拾えず、従来どおり中央のまま）。詳細は
`docs/generator-weekly-design.md`の該当日の追記を参照。

**利用者の決定（2026-09-14）は「自動＋手動の微調整」。** 自動検出が外れた作品を人が直せる手段が要る。
これはUIの操作導線の話ではなく、**検出結果（またはユーザーの上書き）を文書に保存する必要がある**ため、
スキーマ・API側の判断を仰ぎたい。

案（暫定、Claude Codeの提案）：

1. `feature`ページの各`GeneratorItem`（表紙用のitemIdは無いが、`feature`ページのitemIdを表紙のジャケット元として
   参照している。§4参照）に`coverFocusX?: number`（0〜1）のような任意フィールドを持たせ、手動上書きがあれば
   自動検出より優先する。値が無ければ従来どおり自動検出→無ければ中央。
2. ロックの単位は既存の`item`ロックに相乗りできるか（表紙は独立ページだが、focusXの実体はfeature側の作品に
   紐づく値のため）。
3. DBマイグレーションが要るか（JSONBカラムに生えている`content`配下に追加するだけなら不要かもしれない）は
   `lib/generator/model.ts`の検証スキーマ次第と見ている。

UI側（並び順モーダルに準じた「帯を左右にドラッグ」のような操作）はスキーマが決まってから実装する。
現時点では自動検出のみが有効で、上書き手段はまだ無い。
