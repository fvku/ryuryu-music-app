...
3. **メモリと所要時間の受入：Mac実機で完了、iPhoneは未確認。** 2400px×14枚の逐次生成は約6.2〜6.3秒で完了し、対象レンダラーは約337MBから最大410MBだった。1枚ごとの `releaseCanvas` は維持している（失敗時も `finally` で解放、テストあり）。iPhone実機Safariでの上限確認は別途必要。
