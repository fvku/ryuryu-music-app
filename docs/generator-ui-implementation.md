...
> **`app/generator/uipreview-temp/page.tsx` は見た目確認用の一時ファイル**。固定データで実際のワークスペースを描画し、共有DBには触れない。2026-09-09の利用者指示により、Claude側のモバイル再設計と受入が終わるまでは残し、その後の最終公開前に削除する。
...
- `uipreview-temp/page.tsx` は作業中に必要なため保全対象に含めた。現在はモバイル再設計用に意図して残し、再設計と受入後の最終公開前に削除する。
...
- 実共有W36文書のOther Releases 30件を統合画面で確認し、全行が枠内に収まり、折り返し・欠落・行数変化がないこと、
  最終行の`働くサバたち。 / サバシスター`が正常に描画されることを確認した。
- 実共有2026年8月Japan文書で、採用枠の`一張羅 / 小袋成彬, 5lack`、掲載枠の`祝おう`、
  別掲載枠の`°pbdb, 梅井美咲, 北村蕗`を確認した。はみ出し・欠落・コンソール警告はなく、version 12のまま、ロック0件へ戻した。

### 未確認・Koheiへの確認事項

- ウェイトは**作品名400／アーティスト300でKoheiが確定**（2026-09-09）。Monthlyの和文サンプルも受領し、
  Weekly・Japanの両方で裏付けが取れた（当初は「300か400か」を残していたが解消）。
- 実アプリでの採用枠・掲載枠・Other Releasesの和文目視は完了した。
- Monthly／Japanの**掲載枠**の和文（`Japan_2026_2-2.png`の`°pbdb, 梅井美咲, 北村蕗`）は実アプリで目視済みだが、
  参照画像との数値測定はまだ行っていない。採用枠と同じ`TYPE.artistJP`を使うため、必要なら追加で画素比較する。
...
- 2026-09-09、共有DBを変更せず固定データ画面の対象月だけを一時的に2027年1月へ変えて実画面を確認した。
  警告文が表示され、単枚PNG・共有・一括ZIPがすべて無効になり、一括ZIPのtitleにも同じ理由が入った。
  確認後に固定データを2026年8月へ戻した。境界の挙動は単体テストでも固定してある。
...
## 24. 物理iPhone実機の最終受入手順

> **2026-09-09 利用者判断：実施を保留。** モバイル版は現行UIの合否確認を先に行わず、
> UIと提供機能の範囲からClaude側で再検討する。再設計の承認・実装後に、この手順を更新して実施する。

Simulatorとデスクトップブラウザでは確認済みだが、物理iPhoneのSafariだけは端末上で確認する。
共有文書を変更しないよう、まず本番の固定データ画面
`https://ryuryu-music-app.vercel.app/generator/uipreview-temp` を使う。
この画面の確認が完了したら、一時ルートは削除する。
...
- [ ] 縦向きでプレビューと操作欄をスクロールでき、横スクロールが発生しない。
- [ ] 横向きでもプレビューと操作欄へ到達でき、字間の調整UIが表示されず、行送りだけ操作できる。
- [ ] 日本語IMEで変換・確定・削除・取り消しができ、プレビューへ反映される。
- [ ] テキストの選択ハンドルを動かしても、ページスクロールや入力操作が止まらない。
- [ ] 写真ライブラリから画像を選択でき、向きとトリミングが正しくプレビューへ反映される。
- [ ] 編集途中で画面をロックし、30秒後に復帰しても画像と編集内容が表示される。
- [ ] 編集途中で別アプリへ切り替え、30秒後にSafariへ戻っても画像と編集内容が表示される。
- [ ] 2400pxの単枚PNGを3回続けて生成でき、各画像を写真またはファイルへ保存できる。
- [ ] 2400pxの一括ZIPを2回続けて生成でき、Safariが落ちず、完了表示が出る。
- [ ] 生成中に共有シートを開いてキャンセルしても、元画面へ戻って再度生成できる。
- [ ] 操作中に画面を縦横へ2往復しても、プレビューの欠落・固定位置のずれ・操作不能が起きない。
...
## 27. 2026-09-11：Weeklyの白罫、Other Releasesの並び、week37の取りこぼしの原因

利用者からの追加3件。

### (1) ジャケット下の白罫が半透明に見える

`tools/generator-lab/core/layout.mjs` の `WEEKLY.CELLS.panel` を `(200,850) 800×300` から **`(200,856) 800×294`** へ。

原因は塗りと罫の重なり。罫は「各セルの外側6px」に描くので、ジャケット(200,50,800,800)の下罫は y850–856 に来る。ところがパネルのセルが y850 から始まっていたため、あとから描くパネルの塗り（`PANEL_FILL` ＝ 黒60%）がその罫の上に重なり、この1本だけ白が40%まで沈んでいた。上側（y844–850）にはパネルの上罫が描かれていたが、こちらはジャケット画像が上書きするので見えていなかった。Monthlyは隣り合うセルを罫の太さぶん離してあるため同じ問題は起きない。

実物投稿4枚（`reference/2026#36` の W-1・W-2・W-3・W-5、2400×2400）を幅方向の全画素で測り、**白罫は y850–856 の6px1本、パネルの塗りは y856 から**であることを確認して確定した。設計文書§6.1の「罫の実測位置 … y850–856」とも一致する（セル座標の側だけが罫の太さぶんずれていた）。

修正後の描画をブラウザのcanvasで実測し、y850–855が幅方向100%で純白、y856からパネルの塗りになることを確認した。修正前の座標で描くと y850–855 は `102,102,102`（白40%）になる。

### (2) Other Releasesの並び

`lib/generator/source.ts` に `sortWeeklyOthers()` を追加し、`selectWeeklyAlbums` のothers側だけ差し替えた。

- **アルバム（洋楽→邦楽）→ EP（洋楽→邦楽）**、各区分の中は**アーティスト名のa-z順**。実運用のEPは邦楽なので、見た目は利用者の指示どおり「洋楽アルバム→邦楽アルバム→邦楽EP」になる。
- 日付は見ない。洋邦は`洋邦`列で判定し、空欄・想定外の値はその区分の末尾。アーティスト名は小文字のコードポイント比較（記号・数字が先、和文はA-Zの後）。
- featureの並び（`sortAlbums`）は変更していない。

設計文書§1は当初から「洋楽→邦楽」と書いていたが、実装はfeatureと同じ`sortAlbums`（EP→**日付**→アーティスト）を共用していたため、実際には発売日順に並んでいた。これが今回の並び違いの原因。

**この並びが効くのは新しく取り込む文書から。** 既存の文書の並びは`itemIds`に固定されているので変わらない。

### (3) week37で「掲載」が読み込まれない理由

**原因は取り込みが一度きりのスナップショットであること。** 対象週の文書を作った後にRelease Masterの`WEEK`列を変えても、既存の文書へは反映されない。

実データで確認した内容は次のとおり。

- 共有DBのweekly文書 `c3f67801-8f91-4418-8475-802c622d4815`（#37、2026-09-11〜2026-09-18）は **2026-09-10 17:41 UTC に作成され、version 1 のまま**（作成後に一度も保存されていない）。中身は feature 4件、others **7件**。
- いまのRelease Masterで同じ週（2026-09-05〜2026-09-11）を取り込み直すと、`採用` 4件・`掲載` **19件**。差の12件はすべて2026-09-11発売の洋楽で、文書作成後に`掲載`へ変えた行。
- 取り込み側の不具合ではない。`selectWeeklyAlbums`は19件すべてを拾う。日付範囲、`#`列（全行37）、重複除去、60件上限のいずれにも引っかかっていない。

**現状、アプリ側に回復手段が無い。** 取り込みのやり直しは`unique (series, period_start, period_end)`で弾かれ、削除APIも無い。作品の追加は`generator_structure_save`が「各ページの`itemIds`の件数不変」を要求するため、構成保存でも入れられない。§26で追加した「Release Masterから再取得」は既存作品の文字情報を直すだけで、作品を増やせない。

対処案は2つ。どちらも利用者の指示待ち。

1. **その文書を消して作り直す**（共有DBの行を直接削除）。version 1で編集も画像アップロードも無いので失われるものは無い。今週の投稿を出すための最短手。
2. **取り込みのやり直し／作品の追加をAPIとして用意する**（Codex）。恒久策。`WEEK`列の変更を既存文書へ反映できるようにする。
...
### 実ブラウザで確認したこと（2026-09-12、固定データ画面）

`/generator/uipreview-temp`（共有DBを変更しない固定データ画面）を、ローカルプレビューの認証迂回で
`localhost:3458`に起動して確認した。起動設定は`.claude/launch.json`の`generator-preview-3458`。
**共有DBへは何も書いていない。**

- ヘッダーが「共通設定／変更履歴／最新版に更新／Release Master 再読込／全ページを書き出す」の5つになり、
  Release Master系が1本になっていること。
- 画像ごとの操作列（画像 2 ／ 共有DBと一致 ／ この画像を編集）と、その下の「この画像は取り込み後まだ
  保存されていません。」が出ること。編集パネルのタブが「情報修正／背景設定」の2つであること。
- **自動ロックが走らないこと。** 「この画像を編集」を押すまでロックを取りにいかない。押したときは
  固定データの文書IDが共有DBに無いため「対象が見つかりません。」で止まり、編集状態にならない。
- `/generator/<id>/settings`が独立した画面として解決すること（固定データでは文書が無いのでエラー面）。
- 統合した差分ダイアログ。`window.fetch`を差し替えてロックと取り込み差分の応答を模し、
  「作品の増減・区分（実行するとversionに確定）」と「文字情報（下書きへ入り、画像の保存で確定）」の
  2節、既定チェック（追加ON・手で修正済みの削除OFF）、件数、並べ直しのチェック、注記の文言を確認した。
  確認後に`fetch`を戻して再読み込みした。
- 375×812で横スクロールが出ないこと（`scrollWidth === clientWidth === 375`）。
- consoleのエラーは固定データ由来の通信失敗（400／401／404）だけで、描画・Reactのエラーは無い。

### 実共有文書での受入（2026-09-12、weekly W37）

`c3f67801-8f91-4418-8475-802c622d4815`（2026 WEEK 37、表紙＋メイン5＋Others）で確認した。
`202609120001_generator_item_source.sql`は同日、利用者がSQL Editorで適用済み。

- **背景色の候補が実ジャケットから出る。** 画像1（Bloc Party / Anatomy of a Brief Romance）で
  `#735554` `#823439` `#733b3d` `#bf4d58` の4色。いずれも白文字が乗る明るさに収まっている。
- **未設定の画像で「編集」を押すと、候補の先頭が初期値に入る**（`#735554`）。プレビューとサムネイルが
  その場で変わり、ヘッダーが「未保存 1件」、背景設定タブに未保存の印が付く。
- **画像ごとの保存が通る。** 「保存」1つで背景色が確定し、`この画像を保存しました（背景色）。`、
  ヘッダーが「すべて保存済み」、サムネイルの「背」が消えた。
- **画像単位の版表示が実履歴で出る。** `最終保存 version 7 · kohei.fuku0926@gmail.com · 2026/09/12 23:54`。
- **共通設定画面が実文書で動く。** 7枚のサムネイル、当月の波の表示、明示的な編集開始と編集終了。
- **「Release Master 再読込」を実行した**（2026-09-12、利用者の承認を得て）。差分は5件（メインへ2件追加、
  Othersから1件削除、メイン→Othersへ2件移動）で、承認時に見せたものと一致することを実行前に確かめた。
  結果は version 8、7枚・作品32件、メイン5/5・Others27/60。メインの5作品、Othersの末尾へ移った2作品、
  消えた1作品のすべてが意図どおり。履歴に`並び順 / Release Masterから作品構成を取り込み直し`が1行積まれた。

### 背景色は作品が入れ替わった画像では残らない（実測、2026-09-12）

取り込み直しの前に画像1（Bloc Party）へ背景色`#735554`を保存しておいた。取り込み直しでこの作品が
Othersへ移った結果、**その画像のページごと作り直され、背景色は残らなかった**。履歴でもversion 7の行が
`背景を保存（削除された画像）`と表示される。

これはCodexの決定（v2 §8-1以降の「削除・区分移動で不要になったページは落とし、追加で必要なページは
新しいID・背景色未設定で作る」）どおりの動作で、不具合ではない。背景色はその画像のジャケットに合わせて
選ぶものなので、別の作品の画像へ引き継ぐほうが誤りになる。

ただし実行後の案内が「既存作品の修正内容と背景設定は保持されています」と言い切っていたため、
**作品が入れ替わった画像は背景色を設定し直す必要がある**と伝える文面へ直した。

### 実文書で見つけて直した不具合

1. **文字情報が読めないと全体が止まっていた。** 作品の増減はサーバーがRelease Masterを読んで返すのに、
   ブラウザ側の`/api/release-master`が失敗しただけでダイアログを開けなかった。2つを分け、
   取れたほうだけで判断できるようにした。シートの一時的な失敗やレート制限でも同じことが起きる。
   （ローカルプレビューの認証迂回はジェネレーターのAPIにしか効かないため、`/api/release-master`は401になる。）
2. **失効していないのに「編集ロックを失いました」と出ることがあった。** 解放済みのロックへ届いた
   古い応答でも状態表示だけ更新していた。実際に持っていたロックを失ったときだけ知らせる。
3. 共通設定画面の最初の案内が編集画面の文面だった。

### 未確認事項

- 作品（文字情報）の保存は未確認。`source`を載せた保存もまだ一度も通していない。
  ローカルプレビューでは`/api/release-master`が401になり、文字情報の差分そのものを出せないため。
  実環境でGoogleログインした状態なら通る見込みだが、確認はできていない。
- 掲載ページ（作品2件）とOther Releases（最大60件）をまたぐ保存は未確認。
- PNG／ZIPの書き出しは実文書で未実施。
...
## 29. 2026-09-13：文言の変更、ジャケットの取得元、書き出しの条件、ロックの不具合

利用者の指示3件と、実文書での確認で見つけた不具合5件。Codexは同日、
URL取り込みと描画のサーバー経由取得を実装した（`71f2bad`）。

### 利用者の指示

| 指示 | 対応 |
| --- | --- |
| ボタン名を「最新版に更新」「Release Master 再読込」へ | 文言と、それを指す案内文・見出し・文書をまとめて変更 |
| ジャケットは`画像リンク変換`のURLからも取得する | 取得元の優先順を`releaseMasterCover()`へ集約し、Apple Music由来かを問わない |
| 無ければアップロードとURL貼り付けを可能に | ジャケット欄にURL入力を追加。取得はCodexのサーバー経由へ |
| 未保存でも書き出しと読み直しを可能に | 下記 |

### 書き出しと読み直しの条件を変えた

きっかけは「最新版に更新の直後なのに未保存と出て、何も編集していない」という報告。
**原因はTimeの空欄補完**で、読み込み直しのたびに下書きを作るため、編集していなくても未保存になる。
補完自体は意図した機能なので、止める側を変えた。

- PNGとZIPは、画像が壊れる理由（はみ出し・行送り不成立・対象月の波が無い）だけで止める。
  未保存では止めず、「いま画面に出ている内容で書き出します」と添える。
- Release Master 再読込は未保存でも開けて実行できる。既存作品の内容は取り込み直しで変わらないので、
  生き残る作品の下書きはそのまま引き継ぎ、外れる作品のぶんだけ破棄する。
  破棄する下書きがあるときは、削除の行に「未保存あり」を出す。
- ヘッダーの未保存チップに内訳を出す（何が未保存か分からなかったため）。

### 実文書で見つけて直した不具合

1. **状態帯の文が1文字ずつ縦に折り返す。** 操作側を`shrink-0`にしていたため、編集者の表示と
   引き継ぎボタンが並ぶと本文の幅が0まで潰れていた。本文に下限を持たせ、操作側は折り返す。
2. **画像のロックを持ったままでは読み直せない。** DBの規則（`generator_lock`）で`page`と`structure`は
   同時に持てない。読み直しの前に自分のロックを解放し、自分の別端末の分は引き取ってから解放する。
   **他の人のロックには触れない。**
3. **取得した直後のロックを解放できない。** ロックの正本である参照をstateの更新関数の中で
   書き換えていたため、引き取った直後に解放しようとしても「持っていない」と判断され、
   解放の要求が飛ばないまま持ち続けていた。参照を同期的に進めてから描画用のstateへ反映する。
   この1件が2の回避を効かなくしていた。
4. **自分が持っているロックが見えない。** 未保存の画像から離れるとロックを持ったままにする設計なので、
   表示していない画像の分は画面のどこにも出なかった。サムネイルに「編集中」の印を出す。
5. **文字情報が読めないと全体が止まる**（§28に記載済み）。

### 実行した確認

- `npx tsc --noEmit --incremental false`、`npm run lint`、`npm test`、`npm run build`：すべて成功。
- 実共有文書 W37（version 12）で確認した。**この確認では共有DBへ保存していない。**
  - Bandcampの画像URL（`https://f4.bcbits.com/img/a0712120107_10.jpg`）が取り込め、ジャケットが差し替わること。
    確認後に「Release Masterの画像へ戻す」で戻し、保存はしていない。
  - 未保存のままPNGのボタンが押せ、「いま画面に出ている内容で書き出します」と出ること。
  - Release Master 再読込のダイアログが開き、増減0件・文字情報は読み込み不可（401）と出ること。
  - キャンセル後にロックが残らないこと（`locks: []`を確認）。

### 未確認事項

- 文字情報の節に差分が並ぶところ。ローカルプレビューでは`/api/release-master`が401になるため。
- URLから取り込んだジャケットを保存して版に確定するところ。
- 複数人が同時に編集しているときの**実行**（POST）。確認（GET）は2026-09-13のCodex実装で
  ロック不要になり、画像を編集したまま差分を開けることを実文書で確認した。
  実行時は従来どおり他の人のpageロックがあると待たされる。
- **画面を離れてもロックがすぐ返らないことがある。** 実文書で、編集中の画像から一覧へ移動したあとも
  pageロックが残り、heartbeatが止まって3分の期限切れで消えた。解放は送っているはずなので、
  競合か拒否のどちらかが起きている。契約どおり期限切れで自然に解消するため運用上は止まらないが、
  他の人はその間その画像を編集できない。原因の特定は未了。


### 2026-09-13 追記：確認（GET）をロック不要にした（Codex実装、`5f1cc47`）

相談していたpageとstructureの排他について、Codexが案1を採った。`GET /reimport`は元から
`generator_read`とRelease Masterの読み取りだけでロックを要求していなかったため、
UI側が差分を開く前に行っていた画像ロックの解放・別端末ロックの引き取り・`structure`ロック取得を外した。

- **差分の確認中も画像の編集を続けられる。** 実文書W37で、画像0を編集中のまま
  「Release Master 再読込」を開けること、そのとき`structure`ロックが作られないこと（`locks: ["page"]`）を確認した。
- 作品の増減・並べ直しが選ばれた場合だけ、確定直前に自分のitem/pageロックを解放し、
  同じ利用者の別端末のロックを引き取って返してから`structure`ロックを取る。
...
## 32. 2026-09-18：画像を選ぶと編集が始まる形へ（保存状態と操作の統一）（Opus 5・エフォート高）

### 発端

利用者から「『この画像を編集』を押すと『版・編集権・画像の状態が変わりました』と出て、ずっと編集できない」。
実共有文書（W38、`b4830886…`）を読むと、**同じ利用者の画面が並び順（structure）のロックを持ったまま**30秒heartbeatで延長し続けていた。
並び順を保存したあと「編集終了」を押さずにダイアログを閉じると、ロックが返らない。DBはpageとstructureの同時ロックを拒むので、
画像のロック取得は必ず`LOCK_CONFLICT`になり、画面には汎用の409文言しか出ていなかった。

あわせて利用者の指摘：「保存状態、どこが触れるのかわからない」「『この画像を編集』『編集終了』と違うデザインのボタンで状態を管理するので分かりづらい」
「編集できないのに情報修正や背景設定が見えているので混同する」。

### 利用者の決定（2026-09-18）

| 論点 | 決定 |
| --- | --- |
| 編集の開始 | 画像を選んだら、そのまま編集できる（2026-09-12の「押してから編集」を置き換え） |
| 未保存のまま別の画像へ | 「編集内容を保存しますか？」で「保存」「変更を破棄」を選ぶ |
| 未保存のまま編集を終える | 同じ確認（編集終了ボタン自体を廃止し、画像を離れる操作に一本化） |

### 変更したファイルとUI

| ファイル | 変更 |
| --- | --- |
| `app/generator/[id]/GeneratorWorkspace.tsx` | 画像選択で自動の編集開始。編集パネルの先頭を「モード表示」（✎ 画像 N を編集中 ＋ 保存状態 ＋ 同形同幅の「変更を破棄」「保存」）にした。編集できない間は編集部品を出さず、理由と次の操作だけを出す。保存／破棄の確認ダイアログ（画像の切替・並び順・画面内リンク）。並び順はダイアログを開くと自動でロック、閉じると必ず返す。ヘッダーの未保存を画像番号で表示。表紙（作品なし）は背景設定だけを出す |
| `app/generator/[id]/session.ts` | `tryAcquire`（失敗を画面上部へ書かずに返す）、`refreshLocks`（版を進めずロック一覧だけ読み直す）、`pagehide`でのロック解放（再読込・タブを閉じた直後に「自分の別端末が編集中」で3分止まらないように） |
| `app/generator/[id]/workspace-types.ts` | `pageEditBlocker`：画像の編集を止めているロック（他の人／自分の別画面／並び順）を返す純関数 |
| `app/generator/[id]/ItemInspector.tsx` | 保存済みから変えた欄に黄色の枠と「・変更あり」（ジャケットも） |
| `app/generator/[id]/Inspectors.tsx` | 並び順ダイアログに「始められない理由」を出す欄。状態の言葉を「保存済み／未保存」へ |
| `app/generator/ui.tsx` | `Panel`に`highlight`（編集中の紫枠） |
| `lib/generator/__tests__/edit-blocker.test.ts` | 新規（7件） |

### 決めたこと・守ったこと

- **API・DB・保存payload・ロック粒度は無変更。** `requestId`／`expectedVersion`／`clientId`・`token`・`generation`の扱い、30秒heartbeat、3分失効、対象別PATCHの順次保存（v2 §8-2の止め方）はそのまま。
- 自動開始は失敗しても同じ画像で繰り返さない（`editAttempt`がnullに戻ったときだけ取り直す）。止められている間だけ20秒ごとに取り直す。
- 作品ロックの取得は、失敗した作品を覚えて再試行しない。画像のロックを失っている間は取りにいかない（実ブラウザで、失効後に描画のたびロック要求が走るループを見つけて直した。191回/30秒）。
- Time補完やRelease Master読み直しで下書きになった作品は、ロックを持っていないことがある。**保存の直前にその作品のロックを取る**（従来は「ロックがありません」で失敗し得た）。
- 背景色の仮の初期値は、ジャケット候補が編集開始の非同期処理中に届いても拾えるよう参照で持つ。
- 取得中に別の画像へ移った場合、取れたロックはその場で返す（持ち歩かない）。
- 自分の画面に残った並び順ロックは、画像の編集を始める前に返す（今回の不具合の直接の対策）。自分の別画面に残っている場合は「並び順の編集を終わらせる」（引き継いで返す）を出す。

### 実行した確認

- `npx tsc --noEmit --incremental false`、`npm run lint`、`npm test`（本体。当時は`.claude/worktrees`にある別作業のテストも数に入り28ファイル・414件と記録していた）、`npm run build`：すべて成功。
- 実ブラウザ（`generator-preview-3458`、実共有文書「2026 WEEK 37」`c3f67801…`、**保存・復元は行っていない。version 16のまま**）：
  - 画像を選ぶだけで編集が始まり、紫枠・「画像 1 を編集中」「保存済み」が出る。DBのロックは表示中の画像の背景・作品だけ。
  - 作品名を変えると「未保存 1件」、欄に「変更あり」、ヘッダー「未保存：画像 1」、サムネイルに未。
  - 別の画像を押すと「編集内容を保存しますか？」。「変更を破棄」で下書きを捨てて移り、前の画像のロックは返った。
  - 2つ目のタブで同じ画像を開くと「自分の別の画面でこの画像を編集中です」＋「この画面で編集する」。押すと作品ロックごと引き継ぎ、元のタブは次のheartbeatで「編集権が切れました」。
  - 並び順を開くと自動でロック、閉じると画像の編集へ戻り、DBに並び順ロックが残らない。
  - 表紙は背景設定だけを出し、背景色は候補の先頭（`#b89989`）が仮に入る（「背景色 未設定」）。
  - iPhone幅（375px）で横スクロールなし、操作帯は画面内に収まる。

### 未確認事項

- 他の利用者（別アカウント）に止められる表示は、実アカウント2つで確かめていない（純関数のテストのみ）。
- 「保存」経由の確認（共有DBへの書き込み）は依頼範囲外のため実文書で押していない。
- 物理iPhone、Safari／Edge／Brave。

### Codexへ確認・引き継ぐ事項

- 機能契約の変更は無い。`LOCK_CONFLICT`の応答に「誰が・どの対象で」止めているかが入れば、`refreshLocks`の往復を省ける（任意）。

## 33. 2026-09-18：初見でも使える画面へ（言葉・状態表示・文字パネル）（Opus 5・エフォート高）

方針と利用者の決定は[修正方針](./generator-ui-clarity-plan.md)。段1〜3を実装した（§6「未保存をすべて保存」は今回やらない）。
言葉は利用者の指定で「ダウンロード」ではなく「書き出し」。

### 変更したファイルとUI

| ファイル | 変更 |
| --- | --- |
| `app/generator/[id]/GeneratorWorkspace.tsx` | ヘッダー：未保存の画像番号を押すとその画像へ移る。共通設定・変更履歴は文字リンク。「読み込み」メニューに「最新のバージョンを読み込む」（未保存があれば確認）と「Release Masterから取り込み直す」を説明つきで並べた（外を押すと閉じる）。3ステップの案内（閉じると`localStorage`で覚える。「使い方」で出し直せる）。上部の帯は操作の結果だけ（成功・案内は6秒で消える）。一時保存は保存済みと違うときだけ知らせる。一時保存に自動の仮の背景色を入れない。曲数・総尺の読み込み失敗は空欄があるときだけ言う。取り込み待ちの情報は作品の欄の下へ。サムネイルは表紙を自分の背景色だけで数え、持ち主はメンバー名。編集できない間のプレビューのクリックに理由を返す。スマホではプレビューの下端に編集中の帯と「保存」。状態文からversion・共有DBを除いた |
| `app/generator/[id]/ItemInspector.tsx` | 欄の一覧を先に。行を押すとその欄が開き、字間・行間（「文字の詰め・行間」）と「元に戻す／やり直す」がその直下に出る。「調整対象」の選択と✎ボタンを廃止。選んだ欄は左の紫の線、変えた欄は「変更あり」と細い黄の枠。空欄に入力先の案内。「画像に表示」。ファイル選択を日本語のボタンに。ジャケットの印を「取り込んだ画像／差し替えた画像／画像がありません」へ。取り込み待ちの表示と「取り込みを取り消す」 |
| `app/generator/[id]/Inspectors.tsx` | 「以前の保存に戻す」：候補をその対象を保存した時点と取り込んだ時点だけにし、日時とメンバー名で出す。背景色の仮の色に「この色で保存するか選んでください」。並び順の説明からversionを除いた |
| `app/generator/[id]/session.ts` | heartbeatの通信失敗・5xxでロックを手放さない（`LOCK_LOST`・4xxのときだけ）。状態文の言い換え |
| `app/generator/GeneratorPreview.tsx` | 「PNGを書き出す」「アプリで送る」、サイズは文字の補足に。未保存の断りを「保存していない変更も含めて書き出します」の1行へ |
| `app/generator/BulkExportButton.tsx` | 「全ページを書き出す（ZIP）」 |
| `app/generator/PageNavigator.tsx` | 印を語で（未保存／〇〇が編集中／背景色なし）。自分の編集中の印を廃止 |
| `app/generator/[id]/workspace-types.ts` | `recoveryHasChanges`（一時保存が保存済みと違うか）。`targetLabels.page`を「背景色」へ |
| `app/generator/ui.tsx` | 編集中のパネルの枠を細く |
| `lib/generator/__tests__/edit-blocker.test.ts` | `recoveryHasChanges`のテスト3件 |

### 維持した機能契約

- API・DB・保存payload・ロック粒度・履歴は無変更。「読み込み」の2操作は1つにまとめていない（メニューへ畳んだだけ）。
- 未保存でも書き出せる（2026-09-13）。一言添える位置を書き出しボタンの横へ移しただけ。
- 復旧キー・`schemaVersion`・600msデバウンス・拒否条件は維持。書く内容から自動の仮の背景色だけを除いた（保存済みと同じなら書かない）。
- 字間・行送りの数値入力（途中入力・選択置換・クランプ）、`Mixed`、評価文の範囲選択、プレビューからの選択（おすすめ曲番号／曲名の個別選択を含む）は、位置を移しただけで処理は同じ。iPhoneで字間を出さない判定も同じ。
- 取り込み基準（`item.source`）の扱いは同じ。取り消しを「全作品まとめて」から「作品ごと」にした。

### 実行した確認

- `npx tsc --noEmit --incremental false`、`npm run lint`、`npm test`（本体。当時は`.claude/worktrees`にある別作業のテストも数に入り28ファイル・417件と記録していた）、`npm run build`：すべて成功。
- 実ブラウザ（`generator-preview-3458`、実共有文書「2026 WEEK 37」。**保存・復元・一時保存の開く／捨てるは押していない**）：
  - ヘッダー（文字リンク・「読み込み」メニューと説明・ZIP）、3ステップの案内、一時保存の知らせ（利用者がこのタブで直した字間が残っていたため出た）。
  - 文字タブ：作品名の行が開き、字間・行間が直下。プレビューで「Ezra Collective」を押すとアーティストの行が開き、行間120%が出る。
  - 表紙は「背景色」タブだけ。サムネイルに自分の「編集中」の印が出ない。
  - iPhone幅：プレビューの下端に「✎ 画像 0 を編集中／保存済み／保存」。最初の実装ではプレビューの`h-full`で帯がパネルからはみ出したので、`h-full`を3カラムの幅だけに効かせて直した。
  - 開発サーバーでheartbeatが10秒かかって503を返し、1回で「編集できなくなりました」になるのを見つけ、`session.ts`で直した。

### 未確認事項

- 「保存」「以前の保存に戻す」「最新のバージョンを読み込む」を押した後の表示（共有DBに書く／捨てるため）。
- 別アカウントの編集中表示、物理iPhone、Safari／Edge／Brave。
- 共通設定の画面（`ThemeWorkspace`）の言葉は、共通部品（保存済み／以前の保存に戻す）以外は未整理。

### Codexへ確認・引き継ぐ事項

- 機能契約の変更は無い。heartbeatの一時失敗を許すようにしたので、DB側の3分失効が唯一の失効条件になる（従来から正本はDB）。

## 34. 2026-09-18：編集パネルを1枚にし、保存を下端の帯へ（Opus 5・エフォート高）

利用者の指摘：「変更を破棄・保存の紫の囲みがデザイン的に違和感」「文字・背景色を押せば切り替わることが分からず、ただのボタンに見える」。
あわせて「みんなの最新の保存を読み込む」→「最新のバージョンを読み込む」へ改称（利用者の指定）。

### 決定（2026-09-18）

- タブをやめて1枚にする。背景色は1行なのでパネルの一番上に置き、その下に文字の欄を続ける。
- 「変更を破棄」「保存」はパネル下端の帯へ。利用者は「スクロールして探すのは不便」を心配していたため、
  **どの幅でも画面の下端に貼りつき、スクロールしても見える**ことを確認してから決めてもらった。

### 変更したファイルとUI

| ファイル | 変更 |
| --- | --- |
| `app/generator/[id]/GeneratorWorkspace.tsx` | 編集パネルを「見出し → 背景色 → 文字 → 下端の保存の帯」の1枚に。タブ（`panel`状態）と紫の囲み・枠を廃止。帯は変更が無いと「すべて保存済み · 日時 名前」＋「以前の保存に戻す」（上に開く小窓で作品と背景色を選べる）、直すと黄色で「未保存の変更 n件［破棄］［保存］」、仮の色だけなら「背景色が仮の色です［保存］」。スマホ用にプレビューへ重ねていた帯は、下端の帯と重複するので外した。掲載の切り替えを「上段の作品／下段の作品」へ |
| `app/generator/[id]/Inspectors.tsx` | `PageInspector`を1行（カラーピッカー・色の値・候補4つ）へ。`RestoreControl`に`bare`（開閉なしで小窓の中に並べる形） |
| `app/generator/[id]/ItemInspector.tsx` | 独自の縦スクロールを外した（パネル全体でスクロールする） |
| `app/generator/ui.tsx` | `Panel`の`highlight`を廃止し、`padding="none"`を追加（見出しと帯を端まで広げる） |

### 実ブラウザで見つけて直したこと

- 3カラムでも、上の案内・知らせの帯でページが縦に伸び、下端の帯が画面の外に出た → どの幅でも`sticky bottom-0`にした。
- それでも出なかった：パネルの`overflow-hidden`がstickyの基準をパネル内にしていた → 外した（スクロールは中身の列が持つ）。

### 実行した確認

- `npx tsc --noEmit --incremental false`、`npm run lint`、`npm test`（本体。当時は`.claude/worktrees`にある別作業のテストも数に入り28ファイル・417件と記録していた）、`npm run build`：すべて成功。
- 実ブラウザ（W37、**保存はしていない**）：PCで帯が画面下端に出る。作品名を変えると帯が黄色「未保存の変更 1件［破棄］［保存］」になり、「破棄」→確認→元に戻って「すべて保存済み」。スマホ幅でも帯が画面下端に出る。

### 未確認事項

- 「以前の保存に戻す」の小窓から実際に戻す操作（共有DBに書くため）。
- iPad幅（2カラム）での見え方は、スマホ幅とPC幅の間として未確認。

## 35. 2026-09-18：並び順をドラッグで入れ替えられるように（Opus 5・エフォート高）

利用者の依頼「並び順を変更の画面で、ドラッグで入れ替えられるようにしてほしい」。

### 変更したファイルとUI

| ファイル | 変更 |
| --- | --- |
| `app/generator/[id]/Inspectors.tsx` | `StructureDialog`の各行の左端に「⋮⋮」のつまみ。つまみを掴んでドラッグし、落とした位置へ差し込む。Weeklyではメインと Others の間で落とすと入れ替え（落とす先に「ここと入れ替え」）。ドラッグ中は掴んだ行が指に付いて動き、差し込む位置に紫の線。ダイアログの上端・下端に近づくと自動でスクロール。↑↓と「⇄」の選択はキーボード操作のために残した |
| `app/generator/[id]/workspace-types.ts` | `movePageItemTo`（同じ区分の中で指定位置へ差し込む。ページごとの件数は変えない）、`structureDrop`（どの行の上半分／下半分に落としたら何が起きるか） |
| `lib/generator/__tests__/structure-drag.test.ts` | 新規（9件） |

### 決めたこと・守ったこと

- HTML標準のドラッグ＆ドロップはiPhoneの指で動かないため、Pointer Eventsで自作した。ライブラリは足していない。
- 掴めるのはつまみだけ（`touch-action: none`はつまみにだけ付ける）。ほかの場所では今までどおりスクロールできる。
- 同じ区分の中は「入れ替え」ではなく「差し込み」。↑↓は隣と入れ替えるだけなので、遠くへ動かすのに何度も押す必要があった。
- 採用と掲載の間は、今までどおり動かせない（落としても元に戻る）。メインと Others の間は差し込みではなく入れ替え（各ページの件数を変えないDB契約のため。既存の`swapWeeklyFeatureItem`を使う）。
- 保存・ロック・DBは無変更。並べ替えた結果は今までどおり「保存」で確定し、未保存で閉じると保存／破棄を聞く。

### 実行した確認

- `npx tsc --noEmit --incremental false`、`npm run lint`、`npm run build`：成功。
  本体のテスト（`npx vitest run --exclude ".claude/**"`）は25ファイル・302件すべて成功。
  `npm test`だけだと、`.claude/worktrees/`にある別作業のテストまで拾って数が増える（§32〜34の件数はこれを含んでいた）。
- 実ブラウザ（W37）：メイン同士でドラッグすると差し込まれて「未保存」、Othersの「Salt Catchers」をメインの行へ落とすと入れ替わった。
  閉じて「変更を破棄」→ DBの並び・版（version 17）は変わらず、並び順のロックも残っていない。

### 未確認事項

- 物理iPhoneの指でのドラッグ（Pointer Eventsで書いているが、実機では未確認）。
- 採用・掲載のあるMonthly／Japan文書でのドラッグ（純関数のテストのみ）。
