...
## 12. 2026-09-07（第3セッション・続き）：全ページのPNG一括書き出し（UI側のみ）
...
利用者の指示により、**UI・逐次描画・進捗・中止までをこのセッションで用意し、ZIPのまとめ役はCodexへ引き継ぐ。**

### 12-2. 用意したもの

| ファイル | 内容 |
| --- | --- |
| `app/generator/bulk-export.ts` | 新規。`buildArchiveEntries()`（検査→描画の2段、1枚ごとに `releaseCanvas`、中止対応）、`pngFileName()` / `archiveFileName()`、`BulkExportBlocked` / `BulkExportAborted`、**`getArchivePacker()`（未実装。現在は常に `null`）** |
...
- **はみ出しがあるページの扱い**：全ページを先に検査し、**1枚でも警告があれば何も作らずに止め、対象の画像番号と理由をまとめて出す**。
  部分的に書き出して「9枚のはずが7枚」になる方が危険だと判断した。**これは仕様判断なので、Codex側で確認してほしい**（§12-5）。
...
`getArchivePacker()` が `null` の間、ボタンは**「全ページを書き出す（準備中）」で押せない状態**にし、
理由を `title` に出す。まとめ役が入れば、この1関数が値を返すだけで動く。

### 12-5. Codexへの依頼

1. **`getArchivePacker()` の実装。** 旧版 `zip.js`（無圧縮stored のみ・約120行）の移植で足りる想定。外部ライブラリは不要。
   実装したら `lib/generator/__tests__/bulk-export.test.ts` の「まとめ役はまだ実装されていない」テストを消してほしい。
2. **はみ出しページの扱いの確認**（§12-3）。全体を止める／そのページだけ除く、のどちらを仕様にするか。決めた内容は仕様書§7へ追記してほしい。
3. **メモリと所要時間の受入**。2400px×9〜14枚の逐次生成を、iPhone実機で確認してほしい。逐次解放しているが、実機での上限は未確認。
...
- **ブラウザでの見え方は未確認。** 検証には `.next` の再ビルドが要り、3456で動いているプレビューを
  また停止させてしまうため、利用者の判断を待っている状態。ボタンは現在どのみち押せない状態なので、
  実際に動かせる確認はCodexが `getArchivePacker()` を実装したあとになる。
