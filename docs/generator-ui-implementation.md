...
- 「Release Masterから更新」の差分も取れた（下記の不具合を直したあと）。**実行はしていない。**
  内容はメイン5枠の入れ替えを含む編集判断なので、利用者の指示を待っている。
...
- 「Release Masterから更新」の**実行**は未実施（利用者の判断待ち）。
- 作品（文字情報）の保存は未確認。実文書で文字を変えることになるため、背景色だけで通した。
  保存の経路は同じ`saveImage`だが、`source`を載せた保存はまだ一度も通していない。
