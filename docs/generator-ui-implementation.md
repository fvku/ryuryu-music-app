...
- **実共有DBの文書では未確認。** 固定データ画面では文書がDBに無いため、ロック取得・保存・
  取り込み直しの成功経路を通せない。実文書での受入は依頼待ち。
- 背景色の候補は実ジャケットで未確認。固定データにジャケットが無く、候補が出る条件を満たさない。
- 共通設定画面の中身（波の差し替え、合成背景、PNGサイズ）も実文書が要る。
