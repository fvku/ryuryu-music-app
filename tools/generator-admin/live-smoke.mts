...
async function rpc(name: string, values: unknown[]) {
  assert(['generator_create', 'generator_lock', 'generator_save', 'generator_structure_save', 'generator_asset_prepare', 'generator_asset_ready', 'generator_asset_read', 'generator_read'].includes(name));
...
try {
  ...
  console.log(JSON.stringify({ liveCreateSaveRestoreStructureAndAssetLifecyclePassed: true, syntheticDataRolledBack: true, storagePolicies: policies }));
  ...
  console.error(JSON.stringify({ liveSmokeFailed: true, code: (error as {code?:string}).code || 'CHECK_FAILED' }));
  process.exitCode = 1;
