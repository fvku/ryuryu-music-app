...
const Layout = (() => {
  ...
  const WEEKLY = {
    CELLS: {
      ...
      // タイトル・アーティスト・メタ帯をまとめて塗る1枚のパネル。
      // 🔵 2026-09-11実測（reference/2026#36 の W-1・W-2・W-3・W-5、幅方向の全画素）：
      // ジャケットとパネルのあいだの白罫は **y850–856の6px1本だけ**で、パネルの塗りはy856から始まる。
      // 以前は y850・h300 としていたため、パネルの塗り（黒60%）が
      // ジャケット側の下罫（850–856）へ重なり、その1本だけ白が40%に沈んでいた。
      // Monthlyと同じ「セルどうしを罫の太さぶん離し、罫は各セルの外側へ置く」規則に合わせてある。
      panel:  { x: 200, y: 856, w: 800, h: 294 },
    },
